...
import { createShortcuts, matchShortcut, type Action } from "./shortcuts.js"

export function setupTerminal(context: Plugin.Context) {
  ...
  async function run(action: Action) {
    if (!sessionID()) return
    try { await actions[action]() } catch (error) { report(error) }
  }
    ...
    render: () => {
      context.keymap.layer(() => ({
        ...
        commands: shortcuts.map(({ action, bind }) => ({
          ...
          bind, palette: true as const, run: () => run(action),
        })),
