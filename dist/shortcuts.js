...
export function createShortcuts(value) {
    if (value !== undefined && (value === null || typeof value !== "object" || Array.isArray(value))) {
        throw new Error("oc-terminal: keys must be an object");
    }
    const overrides = (value ?? {});
    for (const action of Object.keys(overrides)) {
        if (!Object.hasOwn(defaults, action))
            throw new Error(`oc-terminal: unknown action ${action}`);
    }
    const used = new Set();
    return Object.keys(defaults).flatMap((action) => {
        const bind = Object.hasOwn(overrides, action) ? overrides[action] : defaults[action];
        if (bind === false)
            return [];
        if (typeof bind !== "string")
            throw new Error(`oc-terminal: invalid binding for ${action}`);
        ...
        if (used.has(identity))
            throw new Error(`oc-terminal: duplicate binding ${bind}`);
        used.add(identity);
        return [{ action, bind: [...parts, String.fromCharCode(code)].join("+"), code, modifiers }];
    });
}
export function matchShortcut(sequence, shortcuts) {
    ...
    if (!Number.isInteger(encodedModifiers) || encodedModifiers < 1 || encodedModifiers > 256)
        return;
    const modifiers = (encodedModifiers - 1) & ~(64 | 128);
    const shortcut = shortcuts.find((s) => s.code === baseCode(code) && s.modifiers === modifiers);
    if (shortcut)
        return { action: shortcut.action, press: Number(kitty?.[3] ?? 1) === 1 };
}
